...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        echo ""
        sleep 2
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(arch).tar.gz https://github.com/xinsuiyuandong/3x-ui/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/xinsuiyuandong/3x-ui/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz"
        echo ""
        echo -e "--------------------------------------------"
    ...
    wget -O /usr/bin/x-ui-temp https://raw.githubusercontent.com/xinsuiyuandong/3x-ui/main/x-ui.sh
...
echo -e "----------------------------------------------"
echo ""
echo -e "${green}〔3X-UI〕优化版项目地址：${yellow}https://github.com/xinsuiyuandong/3x-ui${plain}" 
echo ""
echo -e "${green} 详细安装教程：${yellow}https://xinsuiyuandong.blogspot.com/2025/07/3x-ui.html${plain}"
echo ""
echo -e "----------------------------------------------"
echo ""
echo -e "-------------->>>>>>>赞 助 推 广 区<<<<<<<<-------------------"
